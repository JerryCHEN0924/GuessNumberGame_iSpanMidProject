...
@WebFilter(filterName = "FilterLoginCheckByManagerTemp", urlPatterns = {"/QueryAllMembersServletTemp", "/QueryAllGamingRecordServletTemp"}, /* urlPatterns用以設定攔截的路徑(*表示全部路徑) */
...
public class FilterLoginCheckByManagerTemp extends HttpFilter implements Filter { //HttpFilter類別始於JavaEE 8(Servlet 4.0)	
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if(!req.getServletPath().equals(loginFormUrl) && !req.getServletPath().equals(loginProcessingUrl) ) {	
			...
			if(!UserServiceTemp.isManagerAuthenticated(req)) {				
