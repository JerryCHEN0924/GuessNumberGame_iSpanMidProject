...
@WebFilter(filterName = "FilterLoginCheckTemp", urlPatterns = {"/playerPersonalSearchServletTemp", "/Game007Temp.jsp", "/QueryPersonalAllGamingRecordServletTemp", "/QuerryForRankingServletTemp"}, /* urlPatterns用以設定攔截的路徑(*表示全部路徑) */
			initParams = { @WebInitParam(name="login-form-url", value="/_login-form.jsp"),
						   @WebInitParam(name="login-processing-url", value="/Servlet_processLoginTemp")} )
public class FilterLoginCheckTemp extends HttpFilter implements Filter { //HttpFilter類別始於JavaEE 8(Servlet 4.0)	
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if(!req.getServletPath().equals(loginFormUrl) && !req.getServletPath().equals(loginProcessingUrl) ) {	
			...
			if(!UserServiceTemp.isAuthenticated(req)) {				
